...
func NewCmdAttach(comm *koliutil.CommandParams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			comm.Cmd = cmd
			cmdutil.CheckErr(options.Complete(comm, args))
...
func (p *AttachOptions) Complete(comm *koliutil.CommandParams, argsIn []string) error {
	if len(argsIn) == 0 {
		return cmdutil.UsageError(comm.Cmd, "POD is required for attach")
	}
	if len(argsIn) > 1 {
		return cmdutil.UsageError(comm.Cmd, fmt.Sprintf("expected a single argument: POD, saw %d: %s", len(argsIn), argsIn))
	}
	p.PodName = argsIn[0]
