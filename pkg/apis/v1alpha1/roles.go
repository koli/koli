...
func GetRoles() []*rbac.ClusterRole {
	return []*rbac.ClusterRole{
		{
			ObjectMeta: metav1.ObjectMeta{Name: string(RoleExecAllow)},
			Rules:      GetExecRule(),
		},
		{
			ObjectMeta: metav1.ObjectMeta{Name: string(RolePortForwardAllow)},
			Rules:      GetPortForwardRule(),
		},
		{
			ObjectMeta: metav1.ObjectMeta{Name: string(RoleAutoScaleAllow)},
			Rules:      GetAutoScaleRule(),
		},
		{
			ObjectMeta: metav1.ObjectMeta{Name: string(RoleAttachAllow)},
			Rules:      GetAttachRule(),
		},
		{
			ObjectMeta: metav1.ObjectMeta{Name: string(RoleAddonManagement)},
